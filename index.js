...
bot.on('message', message => {
    ...
    if (command === "unwarn") {
        ...
        if (!db.get('warns').find({guild: message.guild.id, user: member.user.id}).value() || db.get('warns').find({guild: message.guild.id, user: member.user.id, warns: 0}).value()) return message.channel.sendCode('', `Erreur : L'utilisateur mentionné n'a aucun avertissements.`);
        let Warns = db.get("warns").filter({guild: message.guild.id, user: member.user.id}).find('user').value();
        let warn = Object.values(Warns);
        db.get('warns').find({guild: message.guild.id, user: member.user.id}).assign({warns: warn[2] -1}).write();
        let embed = new Discord.RichEmbed()
